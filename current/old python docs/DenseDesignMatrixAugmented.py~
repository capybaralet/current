...
class DenseDesignMatrix(Dataset):
    ...
    def __init__(self, X=None, topo_view=None, y=None, stamps=None,
    ...
    def restrict(self, start, stop):
        ...
        if self.stamps is not None:
            self.stamps = self.stamps[start:stop, :]
        assert self.X.shape[0] == self.stamps.shape[0]        
        assert self.X.shape[0] == self.y.shape[0]
...
# This function does not support stamps attribute
    try:
