...
featurelearn = "autoencoder"

numclasses = 10 
patchsize = 6
border = patchsize/2
maxdelta = 5
fixation_mask_size = 4
numhid = 100
...
#SHOW SOME IMAGES
# why the black bar on the right???
...
pcadata, pca_backward, pca_forward = pca(patches.T, .99)
...
for i, image in enumerate(allims):
    ...
    keypoints_with_replacements = numpy.arange(len(patches))[rng.randint(0, len(patches), numpatches_per_image)]
    # To use a consistent number of patch pairs, parse keypoints outside of the loop and replace those that are to close 
    # instead of using the continue statement
    for patch_index_1, patch_index_2 in zip(keypoints_with_replacements[::-1], keypoints_with_replacements[0:]):
...
if featurelearn == "autoencoder":
    model = patchpair_encoder.Patchpairencoder(numvisD=fixation_mask_size**2, numvisY=pca_forward.shape[1], numhid=numhid, 
    ...
    allbigramfeatures = train_autoencoder(model)
    print "done"
else: # assuming k_means
    print "instantiating and training kmeans model"
    featurelearndata = pcadata.T.astype("float32")
    del pcadata
    allbigramfeatures = train_kmeans(featurelearndata)
